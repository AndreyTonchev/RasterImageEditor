...
void PasteCommand::execute() {
    int newWidth = 
        (destImage->getWidth() > posX + srcImage->getWidth()) ?
            destImage->getWidth() :
            posX + srcImage->getWidth();

    int newHeight = 
        (destImage->getHeight() > posY + srcImage->getHeight()) ?
            destImage->getHeight() :
            posY + srcImage->getHeight();

    std::vector<Session::SessionImage>& images = currentSession->getSessionImages();
    for (size_t i = 0; i < images.size(); i++) {
        images[i].image->resize(newWidth, newHeight);
    }
